/*
 *  HID driver for Raydium touchscreens
 *
			       ...
			       char *buf)
{
	...
	struct raydium_data *nd = hid_get_drvdata(hdev);
 ...
 */
static int raydium_input_mapping(struct hid_device *hdev, struct hid_input *hi,
...
static int raydium_input_mapped(struct hid_device *hdev, struct hid_input *hi,
		struct hid_field *field, struct hid_usage *usage,
		unsigned long **bit, int *max)
{
	/* No special mappings needed for the pen and single touch */
 ...
 */
static int raydium_event (struct hid_device *hid, struct hid_field *field,
		                        struct hid_usage *usage, __s32 value)
{
	struct input_dev *input = field->hidinput->input;
	struct raydium_data *nd = hid_get_drvdata(hid);
...
static int raydium_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	int ret;
	struct raydium_data *nd;
	struct hid_input *hidinput;
	struct input_dev *input;
	struct hid_report *report;
	if (id->driver_data)
		hdev->quirks |= HID_QUIRK_MULTI_INPUT;
	nd = kmalloc(sizeof(struct raydium_data), GFP_KERNEL);
	if (!nd) {
...
static void raydium_remove(struct hid_device *hdev)
{
...
static const struct hid_device_id raydium_devices[] = {
...
MODULE_DEVICE_TABLE(hid, raydium_devices);
static const struct hid_usage_id raydium_grabbed_usages[] = {
	{ HID_ANY_ID, HID_ANY_ID, HID_ANY_ID },
	{ HID_ANY_ID - 1, HID_ANY_ID - 1, HID_ANY_ID - 1 }
};
static struct hid_driver raydium_driver = {
	.name = "raydium",
	.id_table = raydium_devices,
	.probe = raydium_probe,
	.remove = raydium_remove,
	.input_mapping = raydium_input_mapping,
	.input_mapped = raydium_input_mapped,
	.usage_table = raydium_grabbed_usages,
	.event = raydium_event,
};
static int __init raydium_init(void)
{
	return hid_register_driver(&raydium_driver);
}
static void __exit raydium_exit(void)
{
	hid_unregister_driver(&raydium_driver);
}
